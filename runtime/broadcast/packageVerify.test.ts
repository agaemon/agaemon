...
describe("verifyAgentProposalExecutionBroadcastPackage", () => {
  it("passes for current saved broadcast package evidence", async () => {
    const evidence = await createBroadcastPackageEvidence();

    await expect(verifyAgentProposalExecutionBroadcastPackage(evidence)).resolves.toEqual({
      passed: true,
      failures: [],
      packageResult: {
        passed: true,
        failures: [],
        package: JSON.parse(evidence.broadcastPackageJson),
      },
    });
  });

  it("rejects mutated raw transaction evidence", async () => {
    const evidence = await createBroadcastPackageEvidence();
    const stalePackage = JSON.parse(evidence.broadcastPackageJson);
    stalePackage.transactions[0].rawTransaction = "0x1234";

    const verification = await verifyAgentProposalExecutionBroadcastPackage({
      ...evidence,
      broadcastPackageJson: JSON.stringify(stalePackage, null, 2),
    });
...
async function createBroadcastPackageEvidence() {
  const evidence = await createBroadcastEvidence();
  const result = await createAgentProposalExecutionBroadcastPackage({
    ...evidence,
    generatedAt: GENERATED_AT,
  });
  if (!result.passed || result.package === null) throw new Error("test fixture broadcast package failed");
  return {
    ...evidence,
    broadcastPackagePath: BROADCAST_PACKAGE_PATH,
    broadcastPackageJson: JSON.stringify(result.package, null, 2),
  };
}

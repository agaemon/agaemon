...
describe("submitAgentProposalExecutionBroadcast", () => {
  it("verifies the package without sending by default", async () => {
    const evidence = await createBroadcastPackageEvidence();
    const calls: string[] = [];

    const result = await submitAgentProposalExecutionBroadcast({
      ...evidence,
      send: false,
      client: {
        getChainId: async () => {
  ...
  it("sends raw transactions sequentially when explicitly enabled", async () => {
    const evidence = await createBroadcastPackageEvidence();
    const packageJson = JSON.parse(evidence.broadcastPackageJson);
    const sent: string[] = [];
    ...
    const result = await submitAgentProposalExecutionBroadcast({
      ...
      client: {
        ...
        waitForTransactionReceipt: async ({ hash }) => ({
          blockNumber: hash === `0x${"34".repeat(32)}` ? 999n : 0n,
          status: "success",
        }),
      },
    });

    expect(sent).toEqual([packageJson.transactions[0].rawTransaction]);
    expect(result).toEqual({
      mode: "send",
      passed: true,
      failures: [],

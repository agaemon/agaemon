...
describe("createAgentProposalExecutionBroadcastCloseoutEvidenceSet", () => {
  it("builds report, archive, and compact status artifacts together", () => {
    const evidenceSet = createAgentProposalExecutionBroadcastCloseoutEvidenceSet({
      ...createReceiptEvidence(),
    ...
    expect(evidenceSet.report.path).toBe(BROADCAST_REPORT_PATH);
    expect(evidenceSet.report.markdown).toContain("# Agent Proposal Execution Broadcast Report");
    expect(evidenceSet.archive.path).toBe(BROADCAST_ARCHIVE_PATH);
    expect(JSON.parse(evidenceSet.archive.json).verification.passed).toBe(true);
    expect(evidenceSet.status).not.toBeNull();
    expect(evidenceSet.status?.path).toBe(CLOSEOUT_STATUS_PATH);
  ...
  it("does not build any artifacts when receipt evidence is stale", () => {

...
const FINALIZATION_ARCHIVE_STATUS_SUMMARY_PACKAGE_STATUS_SUMMARY_PATH =
  "artifacts/example-agent-proposal-execution-broadcast-closeout-finalization-archive-status-summary-package-status-summary.md";
...
describe("verifyPackageStatusSummary", () => {
  it("passes when saved package status summary Markdown matches current package status evidence", () => {
    const evidence = createPackageStatusSummaryEvidence();

    expect(
      verifyPackageStatusSummary(evidence),
    ).toEqual({
      passed: true,
      failures: [],
      expected: evidence.finalizationArchiveStatusSummaryPackageStatusSummaryMarkdown,
    });
  });

  it("rejects edited package status summary Markdown", () => {
    const evidence = createPackageStatusSummaryEvidence();
    ...
    expect(
      verifyPackageStatusSummary({
        ...
        finalizationArchiveStatusSummaryPackageStatusSummaryMarkdown:
          evidence.finalizationArchiveStatusSummaryPackageStatusSummaryMarkdown.replace("passed", "failed"),
      }),
    ).toEqual({
      passed: false,
      failures: [
...
function createPackageStatusSummaryEvidence() {
  ...
  const summary = createPackageStatusSummary(
    summaryParams,
  );
  if (!summary.passed) throw new Error("test fixture package status summary failed");

...
describe("verifyAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatusSummaryPackage", () => {
  it("accepts a saved package manifest that matches current finalization archive status summary evidence", () => {
  ...
  it("rejects a stale saved package summary hash", () => {
    const evidence = createFinalizationArchiveStatusSummaryPackageEvidence();
    const packageManifest = createAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatusSummaryPackage(evidence);
    packageManifest.finalizationArchiveStatusSummary.sha256 = "0".repeat(64);
  ...
  it("rejects unsupported package metadata", () => {
    const evidence = createFinalizationArchiveStatusSummaryPackageEvidence();
    const packageManifest = createAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatusSummaryPackage(evidence);
    packageManifest.schemaVersion = 2 as 1;
    packageManifest.generatedAt = "not-a-date";
    ...
    expect(
      ...
      passed: false,
      failures: [
        "finalization archive status summary package schemaVersion must be 1",
        "finalization archive status summary package generatedAt must be a valid timestamp",
      ],
    });
  });
});

...
describe("verifyAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatus", () => {
  it("accepts a saved finalization archive status that matches current evidence", () => {
    const evidence = createFinalizationArchiveStatusEvidence();

    expect(verifyAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatus(evidence)).toEqual({
      passed: true,
      failures: [],
    });
  });

  it("rejects stale saved finalization archive status JSON", () => {
    const evidence = createFinalizationArchiveStatusEvidence();
    const staleStatus = JSON.parse(evidence.finalizationArchiveStatusJson);
    staleStatus.transactions = 2;

    expect(
      verifyAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatus({
        ...evidence,
        finalizationArchiveStatusJson: formatJson(staleStatus),
      }),
    ).toEqual({
      passed: false,
...
function createFinalizationArchiveStatusEvidence() {

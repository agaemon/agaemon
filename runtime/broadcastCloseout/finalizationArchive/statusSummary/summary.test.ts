...
describe("createAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatusSummary", () => {
  it("renders deterministic Markdown for a verified finalization archive status", () => {
    const summary = createAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatusSummary(
      createFinalizationArchiveStatusEvidence(),
    );
  ...
  it("returns verifier failures for stale finalization archive status evidence", () => {
    const evidence = createFinalizationArchiveStatusEvidence();
    const status = JSON.parse(evidence.finalizationArchiveStatusJson);
    status.transactions = 2;

    expect(
      createAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatusSummary({
        ...evidence,
        finalizationArchiveStatusJson: formatJson(status),
      }),
    ).toEqual({
      passed: false,

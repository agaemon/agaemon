...
describe("verifyAgentProposalExecutionPreview", () => {
  it("passes when the saved preview matches current execution bundle evidence", () => {
    const evidence = createPreviewEvidence();

    expect(verifyAgentProposalExecutionPreview(evidence)).toEqual({
      passed: true,
      failures: [],
      previewResult: {
        passed: true,
        ...
        preview: JSON.parse(evidence.previewJson),
      },
    });
  });

  it("rejects stale or mutated preview transaction summaries", () => {
    const evidence = createPreviewEvidence();
    const preview = JSON.parse(evidence.previewJson);
    preview.transactions[0].dataBytes = 99;

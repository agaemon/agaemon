...
describe("verifyAgentProposalExecutionPackage", () => {
  it("passes when saved package artifacts match current evidence", () => {
    ...
    const verification = verifyAgentProposalExecutionPackage({
      ...evidence,
    ...
    expect(verification.previewVerification.passed).toBe(true);
    expect(verification.runbookVerification.passed).toBe(true);
    expect(verification.packageResult.passed).toBe(true);
  });

  it("uses the saved preview timestamp when recomputing package artifacts", () => {
    const evidence = createBundleEvidence();
    const packageResult = createAgentProposalExecutionPackage(evidence);
    if (!packageResult.passed) throw new Error("test fixture package failed");
    const { generatedAt: _generatedAt, ...evidenceWithoutGeneratedAt } = evidence;

    const verification = verifyAgentProposalExecutionPackage({
      ...evidenceWithoutGeneratedAt,
      previewJson: packageResult.preview.json,
      runbookMarkdown: packageResult.runbook.markdown,
    });
  ...
  it("rejects stale package preview JSON", () => {
    const evidence = createBundleEvidence();
    const packageResult = createAgentProposalExecutionPackage(evidence);
    if (!packageResult.passed) throw new Error("test fixture package failed");
    const preview = JSON.parse(packageResult.preview.json);
    ...
    expect(verification.failures).toEqual([
      "preview transactions do not match current execution bundle",
      "package preview JSON does not match current execution package",
    ]);
  });

  it("rejects stale package runbook Markdown", () => {
    const evidence = createBundleEvidence();
    const packageResult = createAgentProposalExecutionPackage(evidence);
    if (!packageResult.passed) throw new Error("test fixture package failed");

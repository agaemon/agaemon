...
describe("verifyAgentProposalExecutionSigningPayloadPreflight", () => {
  it("returns a passing report for current signing payload evidence", async () => {
    const evidence = await createPayloadEvidence();

    expect(verifyAgentProposalExecutionSigningPayloadPreflight(evidence)).toEqual({
      passed: true,
  ...
  it("returns failed payload checks when saved signing payload evidence is stale", async () => {
    const evidence = await createPayloadEvidence();
    const stalePayload = JSON.parse(evidence.payloadJson);
    stalePayload.transactions[0].gasLimit = "999999";

    const report = verifyAgentProposalExecutionSigningPayloadPreflight({
      ...evidence,

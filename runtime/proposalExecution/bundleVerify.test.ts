...
describe("verifyAgentProposalExecutionBundle", () => {
  it("passes when the saved bundle matches current approved proposal evidence", () => {
    const evidence = createBundleEvidence();

    expect(verifyAgentProposalExecutionBundle(evidence)).toEqual({
      passed: true,
      failures: [],
  ...
  it("rejects stale or mutated bundle transactions", () => {
    const evidence = createBundleEvidence();
    const bundle = JSON.parse(evidence.bundleJson);
    bundle.transactions[0].data = "0xdeadbeef";

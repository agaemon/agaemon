...
import { verifyAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatusSummary } from "../../../../broadcastCloseout/finalizationArchive/statusSummary/verify.js";

export interface BroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliArgs {
...
export interface BroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliOptions {
  argv?: readonly string[];
  writeOutput?: (output: string) => void;
  setExitCode?: (code: number) => void;
  readText?: (path: string) => Promise<string>;
  verifySummary?: (params: unknown) => BroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliResult;
}

interface BroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliResult {
  passed: boolean;
  failures: readonly string[];
}

if (isBroadcastCloseoutFinalizationArchiveStatusSummaryVerifyDirectRun(import.meta.url, process.argv)) {
  await runBroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCli();
}

export async function runBroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCli(
  options: BroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliOptions = {},
): Promise<void> {
  ...
  const verifySummary = options.verifySummary ?? ((params: unknown) =>
    verifyAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatusSummary(
      params as Parameters<typeof verifyAgentProposalExecutionBroadcastCloseoutFinalizationArchiveStatusSummary>[0],
    ) as BroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliResult);

  validateOutputWriter(writeOutput);
  validateArgv(argv);
  const args = parseBroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliArgs(argv);
...
export function parseBroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliArgs(
  argv: readonly string[],
): BroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliArgs {
  validateArgv(argv);
...
export function isBroadcastCloseoutFinalizationArchiveStatusSummaryVerifyDirectRun(moduleUrl: string, argv: readonly string[]): boolean {
...
function validateOptions(options: unknown): asserts options is BroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliOptions {
  if (typeof options !== "object" || options === null || Array.isArray(options)) {
    throw new Error("Broadcast closeout finalization archive status summary verify options must be an object");
  }
}
...
): asserts verifySummary is (params: unknown) => BroadcastCloseoutFinalizationArchiveStatusSummaryVerifyCliResult {
  if (typeof verifySummary !== "function") throw new Error("Broadcast closeout finalization archive status summary verifier must be a function");
}

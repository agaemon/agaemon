...
import { verifyAgentProposalExecutionBroadcastCloseoutFinalizationArchive } from "../../../../broadcastCloseout/finalizationArchive/archive/archiveVerify.js";

export interface BroadcastCloseoutFinalizationArchiveVerifyCliArgs {
...
export interface BroadcastCloseoutFinalizationArchiveVerifyCliOptions {
  argv?: readonly string[];
  writeOutput?: (output: string) => void;
  setExitCode?: (code: number) => void;
  readText?: (path: string) => Promise<string>;
  verifyArchive?: (params: unknown) => BroadcastCloseoutFinalizationArchiveVerifyCliResult;
}

interface BroadcastCloseoutFinalizationArchiveVerifyCliResult {
  passed: boolean;
  failures: readonly string[];
}

if (isBroadcastCloseoutFinalizationArchiveVerifyDirectRun(import.meta.url, process.argv)) {
  await runBroadcastCloseoutFinalizationArchiveVerifyCli();
}

export async function runBroadcastCloseoutFinalizationArchiveVerifyCli(
  options: BroadcastCloseoutFinalizationArchiveVerifyCliOptions = {},
): Promise<void> {
  ...
  const verifyArchive = options.verifyArchive ?? ((params: unknown) =>
    verifyAgentProposalExecutionBroadcastCloseoutFinalizationArchive(
      params as Parameters<typeof verifyAgentProposalExecutionBroadcastCloseoutFinalizationArchive>[0],
    ) as BroadcastCloseoutFinalizationArchiveVerifyCliResult);

  validateOutputWriter(writeOutput);
  validateArgv(argv);
  const args = parseBroadcastCloseoutFinalizationArchiveVerifyCliArgs(argv);
  validateTextReader(readText);
  validateArchiveVerifier(verifyArchive);
...
export function parseBroadcastCloseoutFinalizationArchiveVerifyCliArgs(
  argv: readonly string[],
): BroadcastCloseoutFinalizationArchiveVerifyCliArgs {
  validateArgv(argv);
  const flags = [
...
export function isBroadcastCloseoutFinalizationArchiveVerifyDirectRun(moduleUrl: string, argv: readonly string[]): boolean {
...
function validateOptions(options: unknown): asserts options is BroadcastCloseoutFinalizationArchiveVerifyCliOptions {
  if (typeof options !== "object" || options === null || Array.isArray(options)) {
    throw new Error("Broadcast closeout finalization archive verify options must be an object");
  }
}
...
function validateArchiveVerifier(
  verifyArchive: unknown,
): asserts verifyArchive is (params: unknown) => BroadcastCloseoutFinalizationArchiveVerifyCliResult {
  if (typeof verifyArchive !== "function") throw new Error("Broadcast closeout finalization archive verifier must be a function");
}
